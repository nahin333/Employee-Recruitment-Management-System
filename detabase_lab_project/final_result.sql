...
declare
    cursor c_list is
        select candidate_id from candidate_info where candidate_id in (select candidate_id from post_candidate where post_id in (select post_id from requirement_info where post_name = 'Accountant')) and  qualification = 'Bcom' or qualification = 'Bcom+Mcom' or qualification = 'Bcom+Mcom+PhD' order by points desc;
