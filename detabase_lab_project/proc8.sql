...
declare
    cursor c_list is
        select candidate_id from candidate_info where candidate_id in (select candidate_id from post_candidate where post_id in (select post_id from requirement_info where post_name = 'Lawyer')) and subject ='Law' and  (qualification = 'BA' or qualification = 'BA+MA' or qualification = 'BA+MA+PhD') order by points desc;
